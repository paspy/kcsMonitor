...
class OOIAuthException(Exception):
    def __init__(self, message):
        super(OOIAuthException, self).__init__(message)
...
#        self.connector = aiohttp.ProxyConnector(proxy="http://127.0.0.1:8888", force_close=False)
    ...
    def _get_api_token(self):
        ...
        self.server_token['login_id'] = self.login_id
        #self.server_token['password'] = self.password
        self.server_token['world_id'] = self.world_id
        self.server_token['world_addr'] = self.world_ip
        self.server_token['api_token'] = self.api_token
        self.server_token['result'] = 1
...
@asyncio.coroutine
def CreateAuth(user, password):
    auth = KancolleAuth(user, password)
    try:
...
def main():
    try:
        ...
        r = loop.run_until_complete(CreateAuth(User, Password))
        loop.close()
        ...
        with open(User + '.json', 'w', encoding='utf-8') as f:
        	...
        	print ('Usage: kcsAuthPy.py [username] [password]')
        	return
