...
namespace ElectronicObserver.Data {
    ...
    public class ShipType : ResponseWrapper, IIdentifiable {
        ...
        public override void LoadFromResponse(string apiname, dynamic data) {
            ...
            data = JValue.Parse(Regex.Replace(data.ToString(), @"""(?<id>\d+?)""", @"""api_id_${id}"""));

            base.LoadFromResponse(apiname, (object)data);

            if (IsAvailable) {
                _equipmentType = new HashSet<int>();
                foreach (var type in RawData.api_equip_type) {

                    if ((double)type.Value != 0)
                        _equipmentType.Add(Convert.ToInt32(type.Name.Substring(7)));     //skip api_id_
                }
            }
        }
    }
